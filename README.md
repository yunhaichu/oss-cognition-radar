...
- `--profile-path-preset PATH` / `--profile-path-preset-id ID` / `--profile-path-preset-fixture ID`：用于 `--archive-route-detail`，从 selector preset bundle 批量运行 route detail 导出；可用 preset ID 选择其中一组或多组 selector，也可运行 selector JSON 内置的 validation fixture；如果来源是 dashboard 或 selector fixture status 过滤后的导出，CLI 会保留来源 selector filters、status filter、filtered/unfiltered fixture 计数、fixture validation status 和 expected 匹配结果；导出前会生成 `route_detail_preset_validation_v1` 检查摘要
