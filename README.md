...
- `--profile-path-preset PATH` / `--profile-path-preset-id ID`：用于 `--archive-route-detail`，从 selector preset bundle 批量运行 route detail 导出；可用 preset ID 选择其中一组或多组 selector
