...
python3 radar.py \
  ...
  --db data/radar.sqlite \
  --profile-path-preset reports/route-selectors.json \
  --profile-path-preset-fixture missing_move \
  --archive-output reports/route-detail-preset-fixture.md \
  --json-output reports/route-detail-preset-fixture.json
```
...
- `--archive-route-selectors`：列出可用于 `--archive-route-detail` 的 design move、evidence route 和 repository selector 候选，并在 JSON/Markdown 中附带 `route_detail_selector_preset_bundle_v1` 和带 validation summary/status counts、filtered/unfiltered provenance 的 `route_detail_preset_validation_fixtures_v1`
- `--validation-fixture-status STATUS`：用于 `--archive-route-selectors`，按实际 validation status 过滤内置 fixtures，例如 `blocked`、`duplicate_ids`，同时保留未过滤状态分布
- `--profile-path-preset PATH` / `--profile-path-preset-id ID` / `--profile-path-preset-fixture ID`：用于 `--archive-route-detail`，从 selector preset bundle 批量运行 route detail 导出；可用 preset ID 选择其中一组或多组 selector，也可运行 selector JSON/dashboard 单 fixture bundle 内置的 validation fixture；如果来源是 dashboard 或 selector fixture status 过滤后的导出，CLI 会保留来源 selector filters、status filter、filtered/unfiltered fixture 计数、fixture validation status 和 expected 匹配结果；导出前会生成 `route_detail_preset_validation_v1` 检查摘要
...
- `--archive-dashboard [PATH]`：生成一个可直接打开的静态 HTML dashboard，包含搜索、track 过滤、confidence source 过滤、最低分过滤、跨项目 patterns、项目详情、claims、claim gaps、evidence acquisition bindings、route detail selectors、preset validation fixtures、fixture validation metadata/status 过滤、单个 fixture bundle 下载、带 provenance 的 fixture validation preview 和 evidence 摘要
...
系统还会从 `semantic_v1` patterns 自动派生 `repository_cognition_profiles`。每个仓库画像会显示该项目最强体现的跨项目设计动作、证据族、原始 claim 字段/证据层分布、支撑 semantic patterns 和 evidence examples；`--archive-show` 和 dashboard 的仓库详情页会直接展示 Repository Cognition Profile，`--archive-search` 和 dashboard 搜索也会纳入画像内容。CLI 搜索和 dashboard 仓库详情都会输出 profile-to-claim/evidence explanation paths，把设计动作连接到具体 claim gap、采集原因和 evidence stable ID，并按设计动作、缺口层和 evidence type 汇总 path-level 统计。`--archive-patterns` 和 dashboard 还会输出 `repository_cognition_profile_path_comparisons`，按同一设计动作跨仓库比较不同 claim gap layer 与 evidence type 组成的证据路线；dashboard 可以继续按设计动作、证据路线和仓库下钻这些 comparisons，并在 route detail 面板中展开完整仓库样例、confidence 信号和 evidence stable ID。route detail 面板和 `--archive-route-detail` 都可以按当前过滤范围导出 `route_detail_drilldown_v1` JSON 或 Markdown，把 evidence route、仓库样例、path/pattern ID 和 evidence stable ID 保存为可复用研究材料；`--archive-route-selectors` 和 dashboard JSON 都输出同一版 `route_detail_selectors_v1`，列出批处理可用的设计动作、证据路线和仓库 selector；selector JSON 和 dashboard route detail JSON 都会附带 `route_detail_selector_preset_bundle_v1`，可由 `--profile-path-preset` 直接批量运行 route detail 导出；selector JSON 和 dashboard route detail JSON 也会附带 `route_detail_preset_validation_fixtures_v1`，可用 `--profile-path-preset-fixture` 覆盖 missing move、missing route、missing repo 和 duplicate ID 这类失败路径；直接 `--archive-route-selectors` 输出的 fixtures 也会包含 validation summary、实际 status、expected 匹配结果和状态分布，并可用 `--validation-fixture-status` 按实际 status 过滤；selector Markdown 会同步显示来源、filtered/unfiltered fixture 计数、matching expected 计数和 filtered/all status counts；filtered selector fixture 进入 `--archive-route-detail --profile-path-preset-fixture` 后会继续保留来源 selector filters、filtered/unfiltered fixture 计数和 matching expected 计数；dashboard route detail JSON/Markdown 会把每个 fixture 的 `route_detail_preset_validation_v1` summary、实际 status、expected 匹配结果、来源过滤范围和状态分布一起导出；dashboard 单 fixture bundle 也会保留来源 selector filters、filtered/unfiltered fixture 计数、matching expected 计数和状态分布；CLI 读取 dashboard fixture 导出时会继续保留来源 status filter、实际 validation status 和 expected 匹配结果；dashboard route detail 面板还可以单独下载每个 fixture 的 `route_detail_selector_preset_bundle_v1` JSON/Markdown，并在选择 fixture 时直接预览带 provenance 的 `route_detail_preset_validation_v1` 摘要；preset 批处理会先输出 `route_detail_preset_validation_v1`，汇总 ready/unmatched preset、重复 ID 和预计 route/example 数；dashboard 的设计动作、证据路线和仓库下钻选项会优先使用该 selector payload，并把 selector values 纳入搜索匹配；同一 dashboard 过滤状态会写入 URL hash，Permalink 可以直接恢复搜索、track、confidence、signal、score、设计动作、证据路线、仓库和 fixture status 范围。
