...
- `--archive-route-selectors`：列出可用于 `--archive-route-detail` 的 design move、evidence route 和 repository selector 候选，并在 JSON 中附带 `route_detail_selector_preset_bundle_v1` 和带 validation summary/status counts 的 `route_detail_preset_validation_fixtures_v1`
