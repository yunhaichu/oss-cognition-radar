...
- `--profile-path-preset PATH` / `--profile-path-preset-id ID` / `--profile-path-preset-fixture ID`：用于 `--archive-route-detail`，从 selector preset bundle 批量运行 route detail 导出；可用 preset ID 选择其中一组或多组 selector，也可运行 selector JSON 内置的 validation fixture；导出前会生成 `route_detail_preset_validation_v1` 检查摘要
...
- `--archive-dashboard [PATH]`：生成一个可直接打开的静态 HTML dashboard，包含搜索、track 过滤、confidence source 过滤、最低分过滤、跨项目 patterns、项目详情、claims、claim gaps、evidence acquisition bindings、route detail selectors、preset validation fixtures、fixture validation metadata、单个 fixture bundle 下载、fixture validation preview 和 evidence 摘要
