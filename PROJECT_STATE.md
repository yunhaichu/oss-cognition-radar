...
- Targeted acquisition now ranks candidates by claim field, missing layer, and claim keywords, but evidence-to-claim binding metadata is not yet persisted into archive views or the dashboard.
...
Persist evidence-to-claim acquisition bindings into SQLite/archive views and expose them in the dashboard so each added evidence item shows which claim gap it was collected for.
